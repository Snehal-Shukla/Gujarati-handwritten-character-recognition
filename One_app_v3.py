...
def predict_image(uploaded_file, character_model, character_label_dencoder,
    ...
    if uploaded_file is not None:
        ...
        st.markdown(f"<p style='font-size:37px'><b>Character Prediction : </b> <br> { consonant_label }  +  { vowel_label }  =  { label }</p>", unsafe_allow_html=True)
        st.markdown(f"<p style='font-size:37px'><b>In Gujarati: </b> <br> { consonant_guj_label }  +  { vowel_guj_label }  =  { guj_label }</p>", unsafe_allow_html=True)
        st.markdown(f"<p style='font-size:37px'><b>Confidence : </b> <br> { confidence:.2f}%</p>", unsafe_allow_html=True)
