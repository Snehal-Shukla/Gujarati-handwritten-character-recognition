...
def predict_image(uploaded_file, character_model, character_label_dencoder,
    ...
    if uploaded_file is not None:
        ...
        st.markdown(f"<p style='font-size:37px'><b>Character Prediction : </b> <br> <b>{ consonant_label }  +  { vowel_label }  =  { label }</b></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='font-size:37px'><b>In Gujarati: </b> <br> <b>{ consonant_guj_label }  +  { vowel_guj_label }  =  { guj_label }</b></p>", unsafe_allow_html=True)
        st.markdown(f"<p style='font-size:37px'><b>Confidence : </b> <br> <b>{ confidence:.2f}%</b></p>", unsafe_allow_html=True)
